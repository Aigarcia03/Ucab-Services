...
BEGIN
    ...
    IF LOWER(TG_TABLE_NAME) = 'estudiante' THEN
        v_existe := EXISTS (SELECT 1 FROM Asiste WHERE CIEstudiante = NEW.CI);
    ...
    ELSIF LOWER(TG_TABLE_NAME) = 'profesor' THEN
        v_existe := EXISTS (SELECT 1 FROM Asiste WHERE CIProfesor = NEW.CI);
        IF NOT v_existe THEN
    ...
    ELSIF LOWER(TG_TABLE_NAME) = 'curso' THEN
        v_existe := EXISTS (SELECT 1 FROM Asiste WHERE Materia = NEW.Materia);
        IF NOT v_existe THEN
...
BEGIN
    ...
    IF NOT v_existe THEN
        RAISE EXCEPTION 'Restricción (1,n) violada: El Trámite con CI %, IDPrestadora %, Categoria %, Descripcion % y FechaCreacion % debe estar asociado con al menos un Paso Actividad.', NEW.CI, NEW.IDPrestadora, NEW.Categoria, NEW.Descripcion, NEW.FechaCreacion;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE TRIGGER trg_tramite_crea_cuenta
...
CREATE OR REPLACE TRIGGER trg_before_registra
...
BEGIN
    ...
    IF NEW.Estado = 'finalizado' THEN
        ...
        UPDATE Acompañante AS a
        SET Estado = 'archivado'
        FROM Tramite AS t
        WHERE t.CI = a.CIMiembro
        AND t.IDPrestadora = a.IDPrestadora
        AND t.NombreCategoria = a.NombreCategoria
        AND t.Descripcion = a.Descripcion
        AND t.FechaCreacion = a.FechaCreacion;
    ELSE
        UPDATE Acompañante AS a
        SET Estado = 'activo'
        FROM Tramite AS t
    ...
    ELSE
        ...
        FROM generate_series(NEW.FechaCreacion + 1, NEW.FechaCierre, '1 day'::interval) AS d
        WHERE EXTRACT(ISODOW FROM d) NOT IN (6, 7) -- Excluye Sábado (6) y Domingo (7)
        AND d::date NOT IN (SELECT Fecha FROM feriados); -- Excluye los feriados
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE TRIGGER trg_before_tramite
...
CREATE OR REPLACE TRIGGER trg_before_paso_actividad
...
CREATE OR REPLACE TRIGGER trg_before_paga
...
CREATE OR REPLACE TRIGGER trg_before_monto_mayor_a_deuda_factura
...
CREATE OR REPLACE TRIGGER trg_before_beneficiario
...
CREATE OR REPLACE TRIGGER trg_fin_beneficiario
...
CREATE OR REPLACE TRIGGER trg_before_bloque_horario
...
CREATE OR REPLACE TRIGGER trg_before_zelle
...
RETURNS TRIGGER AS $$
BEGIN
    UPDATE Tasa SET FechaHoraVigencia = NEW.FechaHoraVigencia WHERE FechaHoraFin IS NULL AND FechaHoraVigencia < NEW.FechaHoraVigencia;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE TRIGGER trg_after_tasa
...
CREATE OR REPLACE TRIGGER trg_before_metodo_pago
...
CREATE OR REPLACE TRIGGER trg_before_tarifa
...
CREATE OR REPLACE TRIGGER trg_before_estado_cuenta
...
CREATE OR REPLACE TRIGGER trg_before_linea_cargo
...
CREATE OR REPLACE TRIGGER trg_before_medio_transporte
...
CREATE OR REPLACE TRIGGER trg_after_viaje
...
CREATE OR REPLACE TRIGGER trg_before_miembro
...
CREATE OR REPLACE TRIGGER trg_after_paga_balance
